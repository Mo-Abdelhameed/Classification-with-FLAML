...
async def transform_req_data_and_make_predictions(
    ...
    predictions_arr = predict_with_model(model_resources.predictor_model, data, return_proba=True)
    columns = sorted(model_resources.data_schema.target_classes)
    predictions_df = pd.DataFrame(predictions_arr, columns=columns)
